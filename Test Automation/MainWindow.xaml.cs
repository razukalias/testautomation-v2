...
namespace Test_Automation
{
    public partial class MainWindow : Window, INotifyPropertyChanged
    {
        ...
        private void RefreshComponentPreview()
        {
            ...
            if (nodeType == "AssertView")
            {
                var latestAssertViewExecution = nodeExecutionResults
                    .OrderByDescending(result => result.EndTime ?? result.StartTime)
                    .FirstOrDefault();
                var lastAssertViewData = GetLastExecutionData<Test_Automation.Models.AssertViewData>(nodeId);

                // Build beautiful assertion results display
                var results = lastAssertViewData?.AssertionResults ?? new List<Test_Automation.Models.AssertionResultItem>();
                var totalPassed = lastAssertViewData?.TotalPassed ?? 0;
                var totalFailed = lastAssertViewData?.TotalFailed ?? 0;
                var total = totalPassed + totalFailed;

                // PreviewOutput - summary stats
                PreviewOutput = JsonSerializer.Serialize(new
                {
                    summary = new
                    {
                        ...
                        passed = totalPassed,
                        failed = totalFailed,
                        passRate = total > 0 ? Math.Round((double)totalPassed / total * 100, 1) : 0,
                        durationMs = latestAssertViewExecution?.DurationMs ?? 0
                    },
                    boundComponents = lastAssertViewData?.BoundComponentIds ?? new List<string>(),
                ...
                // PreviewResponse - detailed results
                PreviewResponse = JsonSerializer.Serialize(new
                {
                    assertionResults = results.Select(r => new
                    {
                        id = r.Id,
                        status = r.Passed ? "passed" : "failed",
                        component = r.ComponentName,
                    ...
                    history = lastAssertViewData?.History ?? new List<Test_Automation.Models.AssertionHistoryItem>()
                }, PrettyJsonOptions);

                var failedList = string.Join(", ", results.Where(r => !r.Passed).Select(r => $"{r.ComponentName}: {r.JsonPath}"));
                PreviewLogs = $"[{now}] AssertView preview refreshed\n[{now}] Total: {total}, Passed: {totalPassed}, Failed: {totalFailed}\n[{now}] Failed: {failedList}";
                AppendExtractionPreview(now);
                RebuildPreviewLogsForSelectedNode();
                return;
            }
